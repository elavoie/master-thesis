...
studies and their choices of data to gather, by the availability of language
features that support the reification and by the optimizations performed by the
underlying runtime. Developments in any of those areas will further influence
...
Given our focus on instrumenting JavaScript, we left out the reification of
...
through the \kw{toString} and \kw{valueOf} methods to support proxy
...
having to reify primitive operations to dispatch to our custom objects.} The
...
reliance on \kw{call}~\footnote{When referring to the call method of a
function, code typesetting will be used. When referring to function calls in
general, normal typesetting will apply.} and \kw{apply} native methods and to
default to the native calling protocol as long as they are not modified, in the
interest of speed.
...
steps. We will first discuss the basic operation, that closely corresponds to a
...
\begin{itemize}
    ...
    \item \kw{has(name)}: returns true if the \kw{name} property is present on
        the object itself, without a lookup of the prototype chain
    \item \kw{set(name, value)}: create a new \kw{name} property if not already
    present, and set or update its value to \kw{value} 
    \item \kw{getPrototype()}: returns the prototype of an
    object~\footnote{As explained in the conclusion, using the \kw{\_\_proto\_\_}
...
Unless specified, the pseudo-code follows the regular JavaScript semantics. We
will take advantage of the expressivity of the rest and spread operators, bound
to appear in the next revision~\cite{ECMAScript6}. For presentation simplicity,
primitive values, missing values and error handling are omitted.

\subsubsection{Basic}

The basic version essentially performs a regular lookup followed by a method
call. This is the semantics of a method call in JavaScript as well:
...
causal connection between the state of the \kw{call} method and the behavior
...
indirection to every function call. Before we address performance issues of the
...
different depending on the state of the {Function.prototype}'s \kw{call}
...
Now that the semantics and mapping have been established, we will address the
...
correct method is left to the programmer. That being said, we believe it should
...
\textit{the proxy prototype chain should mirror the native objects' chain}.
...
root proxies need to have an additional method and all other proxies will then
